...
⚠️ **いまアイキャッチ画像の作り直しを途中まで進めている。**`furusato-nozei-jogen.png` だけが
手元と本番で違う（下の節）。**残り8枚を同じやり方に広げるかは判断待ち。**
...
## アイキャッチを読めるようにする作業（2026-09-04・**1枚だけ完了。残り8枚は判断待ち**）
...
**入れた解決策**: PowerPoint のテキストボックスなら日本語を置ける（見出し・凡例・注記が既にそう）。
...
- `at` は **SVG の viewBox 座標**。図を描いた座標系のまま書けるので、SVG を直したときに追従させやすい
- `align` は `"left"`（既定）／`"right"`／`"center"`。`at` がテキストのどちら側かを決める
- ⚠️ **座標変換は x と y で別々の倍率をかけている。**図版は縦横比を保たずに枠へ引き伸ばされているため

### 済んだのは1枚だけ（ふるさと納税）

- 図を viewBox の左半分（x < 200）へ寄せ、右半分をラベルの置き場所にした
- `(1)(2)(3)` の横に「(1) 所得税から」「(2) 住民税の基本分」「(3) 住民税の特例分」を直接置いた
- 破線に「住民税所得割額の20％ ＝ここが天井」を黄色で直接置いた
- **凡例を削除**（ラベルと二重になるため）。注記は1行に短縮
- **意味の説明がなかった上向き矢印を削除**

⚠️ **1回目はラベルが破線に重なった。**破線の右端を x=204 まで縮め、ラベルを y=84 → 74 に上げて直した。
**ラベルの位置は目で見るまで合っているか分からない。**

### 残り8枚（**判断待ち。ユーザーに一覧を出して承認待ちの状態**）

| 記事 | 図に置く予定のラベル |
|---|---|
| 扶養の壁 | 5本の棒に「103 所得税」「106 社会保険」…と**何の壁か**を直接 |
| 高校無償化 | 2つの箱に「授業料」「授業料以外」。**意味不明だった下向き矢印**に「支援金で埋まる」／横線に「残る自己負担」 |
| 大学無償化 | 「授業料等減免」「給付型奨学金」。所得制限が外れた側にその旨 |
| 高額療養費 | 階段に「毎月の自己負担」、破線に「年間上限」、横軸の端に「8月」「翌年7月」 |
| 児童手当 | 3つの箱に「第1子」「第2子」「第3子（加算あり）」、点線の箱に「数に入るが手当は出ない」 |
| 医療費控除 | 3段に「控除の対象」「適用下限額」「保険金などで補てんされた分」 |
| 住宅ローン減税 | 4段に「長期優良・低炭素」「ZEH水準」「省エネ基準適合」「その他住宅」、破線に「支援対象外」 |
| 奨学金 | 4段に「元の返還」「減額返還」「返還期限猶予」「返還免除」（注記の「上から…」がそのまま図に入る） |

**進め方**: 1枚ごとに `build.py` を走らせて **PNG を目で見る**。まとめて8枚作らない。
凡例がラベルと二重になる枚は凡例を消す。

⚠️ **画像を直したら `eyecatchAlt` も直す。**ガードは「書いたか」と「タイトルの丸写しか」しか見ない。
今日、図を描き直したのに alt が古い図の説明のままビルドが通った実例がある。

⚠️ **本番はまだ元の画像のまま。**`furusato-nozei-jogen.png` を手元で作り直しただけで、デプロイしていない。
...
1. **アイキャッチの残り8枚（判断待ち）。**上の節の一覧で承認を待っている状態。
   決まったら1枚ずつ作って目で見る。終わったら `main` へ push（自動デプロイ）
