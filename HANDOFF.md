...
| 広告リンク | **記事6本に置いて公開済み（2026-09-16・本番で実測）。`affiliateEnabled` は `true`。** もしもアフィリエイトで **8件提携中・5件申請中（Amazon 含む）**（下の節） |
...
## もしもアフィリエイト（2026-09-16・**8件提携中・5件申請中・記事6本にリンクを置いて公開済み**）
...
**審査あり → 「申請中」。広告主の判断待ち（5件）**: ポケマルふるさと納税（3830・200円）／ガーデン 学資金の無料相談（5006・11,500円）／
マネイロ（4745）／bloomo 投資アプリ口座開設（6795・3,520円）／**Amazon.co.jp（170・1.8%）**。通ったら原稿取得ページからリンクを取って `links.json` に足す。

**Amazon（170）は開示を出してから申請した（同日）。** 申請直前の注記「メディア内に Amazon との関係を明記する必要があります。審査時点で記載がない場合、提携否認」に合わせ、
`about.md`・`privacy.md`・フッター（`site.json` の `affiliateDisclosure`）に
「Amazonのアソシエイトとして、家計の制度ログは適格販売により収入を得ています。」を入れ、**本番で表示されるのを curl で確認してから**申請した。
⚠️ Amazon の提携申請モーダルは、Chrome 拡張のクリックでは開かないことがある（弥生と同じ症状）。JS で `button.js-affiliate_apply[data-promotion_id="170"]` を click するとモーダルが開き、中の「提携申請する」を押せた。
...
### 記事に置いた場所（2026-09-16・本番で `class="buy"` を実測）

| 記事 | 置いたキー | 節の見出し |
|---|---|---|
| `zeikin/iryouhi-koujo` ／ `zeikin/jutaku-loan-koujo` ／ `shisan/taishoku-shotoku` | `yayoi-kakuteishinkoku`・`mf-kakuteishinkoku` | 申告書を作るときの選択肢（広告を含みます） |
...
⚠️ **`build.mjs` を直した**: PR表記（「この記事には広告が含まれます」）が**全記事**に付いていた。
広告の無い記事に付けるのは事実に反するので、`[[AF:]]` を含む記事だけに出すようにした
（`affiliateEnabled=false` のときは従来どおり全記事に「リンクは無い」の注記）。
`test/guards.test.mjs` に回帰テストを足し、**直す前の `build.mjs` でそのテストが落ちることも確かめた**（38 pass）。
...
2. **広告。** 記事6本にリンクを置いて公開済み。申請中5件（Amazon 含む）の結果待ち。FP相談2件をどこに置くかは判断待ち（もしもの節）
