...
**公開済み。記事は手元・本番とも9本で一致している。**

アイキャッチ画像9枚は作り直して本番反映まで済んでいる（下の節）。
...
## アイキャッチを読めるようにした（2026-09-04・**9枚すべて完了。本番反映まで確認済み**）

**指摘**: 「全体的にイラストがわかりづらい」。

**原因は個々の絵柄ではなく構造。** `CLAUDE.md` の「日本語は SVG に入れない」
（ラスタライズをフォントに依存させないため）を守った結果、**図の中の要素に名前が付けられず**、
読者は「図形 → 右下の小さな凡例 → さらに小さい注記」と**3往復**しないと図を読めなかった。

**解決策**: PowerPoint のテキストボックスなら日本語を置ける（見出し・凡例・注記が既にそう）。
`build.py` に **図の要素のとなりにラベルを置く仕組み**を足した。**SVG には日本語を入れていないので決まりは守っている。**

| 足したもの | 中身 |
|---|---|
| `FIG_L / FIG_T / FIG_W / FIG_H` | 図版を置く枠を定数化。画像とラベルの座標変換を必ず一致させるため |
| `svg_viewbox()` | SVG の viewBox を読む。ラベルの座標変換に使う |
| `add_figure_labels()` | `spec["labels"]` を **SVG の viewBox 座標**で受けてラベルを置く |
...
- `align` は `"left"`（既定）／`"right"`／`"center"`。`at` がテキストのどちら側かを決める
- `width` で箱の幅（既定 `LABEL_BOX_W` = 1.9インチ）。超える文字数は折り返す
- ⚠️ **座標変換は x と y で別々の倍率をかけている。**図版は縦横比を保たずに枠へ引き伸ばされているため

### 9枚に何を置いたか

| 記事 | 図に入れたラベル | 凡例 |
|---|---|---|
| ふるさと納税 | (1) 所得税から／(2) 住民税の基本分／(3) 住民税の特例分／住民税所得割額の20％＝ここが天井 | 消した |
| 扶養の壁 | 壁の下に 所得税（〜令和6年分）／社会保険の適用拡大／健康保険の被扶養者／配偶者特別控除／所得税（令和7年分〜） | 消した（注記に色分けを1行で書いた） |
| 高校無償化 | 高校でかかる費用／授業料／授業料以外／高等学校等就学支援金／高校生等奨学給付金 | 消した |
| 大学無償化 | 高等教育の修学支援新制度／授業料等減免／給付型奨学金／多子世帯は所得制限なし／収入の基準が残っている | 消した |
| 高額療養費 | 年間上限／実際に払う自己負担／払わずに済む分／8月／翌年7月 | 消した |
| 児童手当 | 1人目／2人目／3人目＝加算の対象 | 消した |
| 医療費控除 | 控除の対象になる分／ここまでが引かれる／適用下限額／保険金などで補てんされた分 | 消した |
| 住宅ローン減税 | 長期優良住宅・低炭素住宅／ZEH水準省エネ住宅／省エネ基準適合住宅／その他住宅 新築は支援対象外 | **残した**（1段が新築・既存の2本なので色分けが要る） |
| 奨学金 | 元の返還／減額返還／返還期限猶予／返還免除 | **残した**（返す分・待つ期間の色分けが要る） |

**SVG を動かしたのは2枚だけ**（`iryouhi-koujo` は棒を左へ寄せて右をラベル置き場に、
`shogakukin-henkan` は viewBox を 420 → 520 に広げて右を空けた）。ふるさと納税は前の段で寄せ済み。

### 実際にやり直した失敗（同じことを繰り返さないため）

- **ふるさと納税**: ラベルが破線に重なった → 破線の右端を x=204 まで縮め、ラベルを上げた
- **高校無償化**: 注記が折り返して「（円）」だけ2行目に落ちた → 注記を自分で2行に割った
- **高額療養費**: `実際に払う自己負担`（NAVY）が白い階段からはみ出して**紺地に紺の字**になり読めなかった
  → 階段の内側（x=250, y=355）へ移した
- **医療費控除**: `ここまでが引かれる` に破線が食い込んだ → 破線を x=302 で止め、ラベルを x=318 へ

⚠️ **ラベルの位置が合っているかは PNG を目で見るまで分からない。**9枚とも1枚ずつ見て直した。

### `eyecatchAlt` も6本直した

ラベルが増えたので、alt を「図に何と書いてあるか」を含む形に書き直した
（扶養の壁・ふるさと納税・高校無償化・大学無償化・高額療養費・児童手当）。
⚠️ **ガードは「書いたか」と「タイトルの丸写しか」しか見ない。**画像を直したら alt も直す。

### 検証（実際の出力）

```
npm run build → built: 9 article(s), 2 page(s), 2 category page(s)
npm test      → ℹ tests 37 / ℹ pass 37 / ℹ fail 0
python tools/verify-quotes.py → 合計 258 行 / 一致 258 / 不一致 0

本番（main への push から自動ビルド）:
og 画像10枚すべてを手元とバイト比較 → 全一致
alt を直した6記事の HTML を手元ビルドと比較 → 差分0
```
