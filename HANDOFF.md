...
- **公開**: 週2本。題材は本人GOのみ執筆。需要が薄い週でも候補提示は止めない。
- **Search Console**: 数値取得もインデックス依頼も **boxブラウザ**（`sc-domain:nexeed-lab.com`）。セッションはサインアウトしない。Ryzeは使わない。
- **インデックス依頼**: Indexing APIは一般ページ不可。公開・大幅更新後はURL検査→「インデックス登録をリクエスト」。sitemapはbuild生成。保証・無制限クォータなし。
- **ルーチン**: 月曜週次（候補・GSC・依頼）／平日健全性（死活＋直近URLの依頼）。
