...
⚠️ **アイキャッチ画像9枚すべてを作り直した（下の節）。**手元と本番で画像が違う。**デプロイが要る。**
...
## アイキャッチを読めるようにした（2026-09-04・**9枚すべて完了。本番未反映**）
...
1. **アイキャッチ9枚を `main` へ push する。**push すれば自動デプロイされる。
   出たら本番の og 画像9枚を手元とバイト比較して一致を確認する
