...
## ⚠️ サイトレビューの未処理指摘（2026-09-01・**未修正**）

`docs/site-review-2026-09-01.md` に全文。**本番と手元ビルドが11ファイル差分0**であることを
確認したうえで、一次情報30本を取得し直して照合した結果。**まだ1つも直していない。**

**事実に反しているもの（先に直す）**:

1. **`/privacy/` が「アクセス解析は導入していません」と書いているのに、そのページ自身が
   Cloudflare Web Analytics のビーコンを読み込んでいる**（本番の出力で確認）。
   affi のレビューで見つけたのと同じ誤りが再発している
2. **ふるさと納税の記事の引用が原文と違う。**`(1)、(2)及び(3)の3つの控除を合計しても…` は
   原文では **`(3)'`**。天井に当たった場合の話なので、`'` が無いと記事の他の記述と矛盾する。
   引用88行を機械照合して**87/88 一致・不一致はこの1件**

⚠️ **引き継ぎに「引用行 37 / 一致 37 / 不一致 0」と書いてあったが、実際には1件不一致がある。**
前回の照合が実体参照（`&#39;`）を戻していなかったため。**照合スクリプトを
`tools/` に置いて再実行できるようにする**（レビューの D3）。

そのほか: 原文の限定句の欠落1件（130万円の「半分未満」の例外）、言い切りが原文より広い箇所1件、
計測の食い違いを止めるガードが無い、アイキャッチの alt が記事タイトルと同じ、スキップリンクが無い、
`site.json` のコメントが実態と食い違っている（`⚠️ 未発行` `⚠️ 未作成` が両方とも古い）。
