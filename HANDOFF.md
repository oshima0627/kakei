...
**公開済み。記事は手元・本番とも13本で一致している。カテゴリを5つに増やし、題材の範囲を広げた（下の節）。**
...
| 記事 | 手元 **13本** / 本番 **13本**（下の表。13本すべて本番反映済み） |
| カテゴリ | **5つ**（`zeikin` 税と社会保険5本 ／ `kyoikuhi` 教育費5本 ／ `nenkin` 年金1本 ／ `shisan` 資産形成1本 ／ `sozoku` 相続1本） |
| sitemap | 手元ビルド **22URL** / 本番 **22URL**（どちらも実測） |
...
1. **⚠️ 2026-10-01（あと24日）に `zeikin-fuyou-no-kabe.md` の `revisionAt` が切れて、
   ...
   対処は「出典を取り直して本文を確認し、`checkedAt` と `revisionAt` を両方更新する」
2. **もしもアフィリエイトの審査結果を見る。** 通ったら提携申請。
   上の表のうち、記事に自然に置けるものから出す。**判断待ち**（どれに申請するか）
...
5. **Search Console のインデックス登録リクエスト**（本人のダッシュボード操作）。
   新しく出た `/sozoku/`・`/nenkin/`・`/shisan/` の3カテゴリと記事3本も対象
6. **記事を増やす。** 広げた3カテゴリはまだ各1本。次の候補（一次情報が揃っているもの）:
   国民年金の学生納付特例（日本年金機構）／老齢年金の繰上げ・繰下げ（同）／
   相続税の基礎控除と小規模宅地等の特例（国税庁）／退職所得控除（国税庁）／
   児童扶養手当（こども家庭庁・`養育費保証PLUS` に接続できる）
